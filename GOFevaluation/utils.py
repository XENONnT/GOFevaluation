...
def plot_equiprobable_histogram(data_sample, bin_edges, order=None,
    ...
    be=limit_bin_edges(bin_edges,data_sample,xlim,ylim,order)
    be_first=be[0]
    be_second=be[1]
    ...
    numhits=False
    try:
        numhits=kwargs.pop('numhits')
    except KeyError:
        pass
        
    hitdensity=False
    try:
        hitdensity=kwargs.pop('hitdensity')
    except KeyError:
        pass
    ...
    if(hitdensity):
        ns=density_modifier(ns,be_first,be_second,data_sample,xlim,ylim)
...
def limit_bin_edges(bin_edges,data_sample,xlim,ylim,order):
    be=[]
    if len(data_sample.shape) == 1:
        i = 0
        bin_edges[0] = xlim[0]
...
def density_modifier(ns,be_first,be_second,data_sample,xlim,ylim):
    if len(data_sample.shape) == 1:
        i=0
        be_first[0] = xlim[0]
        be_first[-1] = xlim[1]
